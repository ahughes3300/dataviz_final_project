---
title: "US Birthrates for 2000-2014"
...
> Creating a new dataframe for birth month totals by year

```{r}
year <- c(2000)
month <- c(1)
births <-c(2)
brith_month_totals_by_year <- data.frame(year, month, births)

for (years in 2000:2014) {
  for(month in 1:12){
    summer <-sum(birhtrate_cvs$births[birhtrate_cvs$month==month & birhtrate_cvs$year ==years])
    brith_month_totals_by_year[nrow(brith_month_totals_by_year) + 1,] <- list(years, month, summer)
    }
}
brith_month_totals_by_year = brith_month_totals_by_year[-1,]
```

> Getting the top month for each year and adding it to the dataframe birthrate by year
...
> Basic Line Graph for General Outline

```{r}
graph1<-ggplot(data = birhtrate_cvs, aes(x = year, y = births))  +  
  stat_summary(fun = sum, geom="line", colour = "red", linewidth = 1)+
  labs(title = "Birthrates From 2000-2014", colour ="Top Month")+xlab("Year 2000-2014")+ylab("Total Births for Year")
graph1
```

> Improved Line Graph changed the color and added points for the top month

```{r}

graph2<-ggplot(birthrate_by_year, aes(x = year, y = births, color=top_month)) +
  geom_line(color='steelblue4')+scale_x_continuous(breaks = birthrate_by_year$year)+
  scale_color_discrete(name  ="Top Month")+
  geom_point()+scale_color_brewer(palette="Dark2")+labs(
  title = "Birthrates From 2000-2014 with Top Month", colour ="Top Month")+xlab("")+ylab("Total Birts per Year")
graph2
```

> Creating a new dataframe for births per month with totals by day and percent of births each day
...
> Create a graph that shows birth perctange by day for each year
...
> Create a lollipod graph for August brithrates each day for years 2005-2007

```{r}
graph4<- ggplot(aug_birth3,
       aes(x = date_of_month, y = births, 
           color = day_of_week)) +
